...
export function Header() {
  ...
  const scrollToSection = (id: string) => {
    ...
    // Mapeamento direto de IDs alternativos para garantir que sempre encontre uma secao
    const targets: Record<string, string[]> = {
      solucoes: ["solucoes", "modelos", "inicio"],
      planos: ["planos", "depoimentos"],
      faq: ["faq", "orcamento"],
    };

    const idsToSearch = targets[id] || [id];
    let element: HTMLElement | null = null;

    for (const searchId of idsToSearch) {
      element =
        document.getElementById(searchId) ||
        document.querySelector(`[data-section="${searchId}"]`);
      if (element) break;
    }
